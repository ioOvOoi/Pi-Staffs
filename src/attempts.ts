    ...
    for (const entry of receipt.attempts ?? []) {
      ...
      const id =
        ...
        attemptRole +
          ...
          ":" +
          (entry.attempt ?? 1);
      // 守卫已下沉进 upsertAttempt（settled/owner 防覆盖是状态机不变量），按 applied 记账。
      const outcome = upsertAttempt(
        state,
        {
          id,
          role: attemptRole,
